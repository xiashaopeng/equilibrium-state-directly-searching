...
module DEPLETION
    ...
    subroutine Seek_Equilibrum(mm)!m表示大循环次数
        ...
        dimension FN_NUCL(13),FERROR(7),FERROR_OLD(7)
        dimension FEED(20)
...
subroutine KEY(FEED)
	...
	dimension NO(429),FNO(429)  !---数组NO和FNO分别代表用于临界计算的核素编号和物质的量
...
!----------------------------------------------------------------------------------------  
   ...
   !             if(i.gt.429)exit
   !             if(NO(i).eq.nza)then
   !                 if(FNO(i).ne.0.0) fpu8=fac/FNO(i)
   !             exit                
   !             endif
   !             i=i+1
   !         end do
